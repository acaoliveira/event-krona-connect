...
import { ListFilter, Grid, Calendar as CalendarIcon, Search, MapPin, Users } from 'lucide-react';
import { Link } from 'react-router-dom';

// Mock data for events - reusing from EventListing
const mockEvents = [
  {
    id: '1',
    ...
    <DashboardLayout>
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-kpmg-blue mb-2">Dashboard</h1>
            <p className="text-gray-600">
              Gerencie eventos, inscrições e visualize o calendário da KPMG
            </p>
          </div>
                    ...
                    <div className="flex flex-col md:flex-row">
                      ...
                      <div className="md:w-3/4 p-6">
                        ...
                        <div className="pt-4 flex gap-2">
                          ...
                          <Link to={`/dashboard/eventos/${event.id}/gerenciar`}>
                ...
                filteredEvents.map(event => (
                  <Card key={event.id} className="overflow-hidden border-0 shadow-md hover:shadow-lg transition-shadow">
                    ...
                    <CardContent className="p-6">
                      <h3 className="text-lg font-semibold text-kpmg-blue mb-3">{event.title}</h3>
                      <div className="space-y-2">
                        <div className="flex items-center text-sm text-gray-600">
                          <CalendarIcon className="mr-2 h-4 w-4" />
                          <span>{new Date(event.date).toLocaleDateString('pt-BR')} | {event.time}</span>
                      ...
                      <div className="flex gap-2 mt-4">
                        <Link to={`/dashboard/eventos/${event.id}`} className="flex-1">
                          <Button className="bg-kpmg-blue hover:bg-kpmg-lightblue w-full">Ver</Button>
                        </Link>
                        <Link to={`/dashboard/eventos/${event.id}/gerenciar`} className="flex-1">
